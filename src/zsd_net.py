...
class ZSD(nn.Module):
    def __init__(self, num_classes,
        ...
        self.loc_stage = nn.Sequential(nn.Conv2d(1024, 20, 1, 1, 0, bias=False), nn.BatchNorm2d(20),
                                       ) # nn.LeakyReLU(0.1, inplace=True)
        self.semantic_stage = nn.Sequential(nn.Conv2d(1024, 320, 1, 1, 0, bias=False), nn.BatchNorm2d(320),
                                             )  # nn.LeakyReLU(0.1, inplace=True)

        self.conf_stage = nn.Conv2d(1024+320+20, 5, 1, 1, 0, bias=False)
    ...
    def forward(self, input):
        ...
        output = torch.cat((output_1, output_2), 1) # output1 和 output2 连接起来
        output_Tf = self.stage3_conv1(output)  # [1,1024,13,13] # 特征向量

        output_Tl = self.loc_stage(output_Tf) # 定位向量
        output_Ts = self.semantic_stage(output_Tf) # 语义向量

        output_cat = torch.cat((output_Tf,output_Tl,output_Ts),1)
        ourput_Tc = self.conf_stage(output_cat) # 置信度向量

        # output = self.stage3_conv2(output1024)  # [1,125,13,13]

        return output_Tf,output_Tl ,output_Ts,ourput_Tc
